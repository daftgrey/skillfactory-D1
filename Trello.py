import sys
import requests

# Данные авторизации в API Trello
auth_params = {
    'key': "14a72a530b7dc5222fec834f99e0f67b",
    'token': "bf760a583e679d8071f24feeede1429a0dad3878a0279482dc4b7ee45b7a39ad", }

# Адрес, на котором расположен API Trello, # Именно туда мы будем отправлять HTTP запросы.
base_url = "https://api.trello.com/1/{}"
board_id = "HtjYuA3Y"


def read():
    # Получим данные всех колонок на доске:
    column_data = requests.get(base_url.format('boards') + '/' + board_id + '/lists', params=auth_params).json()

    # Теперь выведем название каждой колонки и всех заданий, которые к ней относятся:
    for column in column_data:
        # Получим данные всех задач в колонке и перечислим все названия
        task_data = requests.get(base_url.format('lists') + '/' + column['id'] + '/cards', params=auth_params).json()
        print(column['name'] + " (Колличество: " + str(len(task_data)) + ")")
        if not task_data:
            print('\t' + 'Нет задач!')
            continue
        for task in task_data:
            print('\t' + task['name'])


def create(name, column_name):
    # Получим данные всех колонок на доске
    column_data = requests.get(base_url.format('boards') + '/' + board_id + '/lists', params=auth_params).json()

    # Переберём данные обо всех колонках, пока не найдём ту колонку, которая нам нужна
    for column in column_data:
        if column['name'] == column_name:
            # Создадим задачу с именем _name_ в найденной колонке
            requests.post(base_url.format('cards'), data={'name': name, 'idList': column['id'], **auth_params})
            break


def move(name, column_name):
    # Перемещение карточки и обработка совпадений.
    tasks_names = {}
    column_names = {}
    column_data = requests.get(base_url.format('boards') + '/'
                               + board_id + '/lists',
                               params=auth_params).json()
    for column in column_data:
        column_names[column['id']] = column['name']
        column = requests.get(
            base_url.format('lists') + '/' + column['id'] + '/cards',
            params=auth_params).json()
        for card in column:
            if card['name'] == name:
                tasks_names[len(tasks_names) + 1] = card
                print('{}.'.format(len(tasks_names)),
                      '{}'.format(card['name']),
                      'in {}.'.format(column_names[card['idList']]),
                      'Description: {}.'.format(card['desc']))
            else:
                break
    i = int(input('Найдены одноименные задачи. Выберите нужную: '))
    for column in column_data:
        if column['name'] == column_name:
            requests.put(base_url.format('cards') +
                         '/' + tasks_names[i]['id'],
                         data={'idList': column['id'], **auth_params})
    print('Карточка передвинута!')


def create_column(name):
    # Создание новой колонки
    requests.post(base_url.format('boards/' + board_id + '/lists'),
        data={'name': name, 'idBoard': board_id, **auth_params})


if __name__ == "__main__":
    if len(sys.argv) <= 2:
        read()
    elif sys.argv[1] == 'create':
        create(sys.argv[2], sys.argv[3])
    elif sys.argv[1] == 'move':
        move(sys.argv[2], sys.argv[3])
    elif sys.argv[1] == 'create_column':
        create_column(sys.argv[2])